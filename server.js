...
app.post("/ask", upload.single("image"), async (req, res) => {
  ...
  const file = req.file; // uploaded referencebillede (valgfrit)
  ...
  try {
    ...
    // ---------- VIDEO ----------
    if (p.includes("video")) {
      // Placeholder video indtil Runway virker
      return res.json({
        type: "video",
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "Lav cinematic AI video prompts. Inkluder kamera, lys, stemning. Kun output prompt."
          },
      ...
      const job = await axios.post(
        "https://api.runwayml.com/v1/tasks",
        {
          ...
          duration: 12,
          ratio: "16:9",
          reference_image: file ? fs.readFileSync(file.path, { encoding: "base64" }) : undefined
        },
        {
          headers: {
