...
app.post("/ask", async (req, res) => {
  try {
    ...
    if (req.body.type === "video") {
      // 1️⃣ ChatGPT til at generere video-prompt
      const completion = await openai.chat.completions.create({
        model: "gpt-4o",
        temperature: 0.2,
        messages: [
...
app.get("/video-status/:jobId", async (req, res) => {
  try {
    ...
    const status = await axios.get(`https://api.dev.runwayml.com/v1/tasks/${jobId}`, { // ✅ Rigtigt endpoint
